(() => {
  window.document
    .querySelectorAll('[apply-hrz-padding]')
    .forEach((container) => {
...
async function setTableAttributes(title) {
  ...
  const addAction = `/system/administracao/dashboard/add/${title.substring(
    0,
    title.length - 1,
  )}`;
  const getAction = `/system/administracao/dashboard/get/php/${title}/index.php`;
  const updateAction = `/system/administracao/dashboard/update/${title.substring(
    0,
    title.length - 1,
  )}`;
  ...
  const searchInput = window.document.querySelector(
    `input[${title}-table-search-input]`,
  );
  const downloadTableButton = window.document.querySelector(
    `button[download-${title}-table-button]`,
  );

  const addLink = window.document.querySelector(
    `a[add-${title.substring(0, title.length - 1)}-link]`,
  );
  addLink.setAttribute('href', addAction);
  addLink.innerText = `+ ${title
    .substring(0, 1)
    .toUpperCase()}${title.substring(1, title.length - 1)}`;
  ...
  async function renderTable() {
    ...
    if (!isTheadRendered) {
      ...
      Object.keys(res.data[0]).forEach((key) => {
        ...
        const usableTableHeadTemplate =
          tableHeadTemplate.cloneNode(true).content.children[0];
    ...
    res.data.forEach((user) => {
      const usableTableBodyTemplate =
        tableBodyTemplate.cloneNode(true).content.children[0];
      const itemTemplate = usableTableBodyTemplate.querySelector('template');

      usableTableBodyTemplate
        .querySelector('a')
        .setAttribute('href', `${updateAction}?id=${user.id}`);
      ...
      Object.keys(user).forEach((key) => {
        ...
        if (!usableTableBodyTemplate.getAttribute('content')) {
          usableTableBodyTemplate.setAttribute(
            'content',
          ...
          usableTableBodyTemplate.setAttribute(
            'content',
            `${rowContent};; ${user[key].toLowerCase()}`,
          );
        }

        const usableItemTemplate =
          itemTemplate.cloneNode(true).content.children[0];
